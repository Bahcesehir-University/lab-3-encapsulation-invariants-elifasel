...
public:
    ...
    void setCelsius(double celsius) {
        // TODO: Implement
        if (celsius < -273.15){
         throw invalid_argument ( " Temperature cannot be below absulute zero" ) ;
    }
    celsius_= celsius ;
 }
};
...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if (initialBalance<0 ){
             throw invalid_argument("Balance cannot be less then zero");
        }
    ...
    void withdraw(double amount) {
        // TODO: Implement
        if ( amount <= 0){
            throw invalid_argument("Wİtdrawal amount must be positive");
        }
        if ( amount > balance_){
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if (pwd.length() < 8 ) {
            throw invalid_argument("Password lenght cannot be less than 8");
        }
        if (!hasDigit(pwd)){
            throw invalid_argument("Password must contain at least one digits");
        }
    }
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if (oldPassword != password_){
            throw invalid_argument(" Old password not the same");
